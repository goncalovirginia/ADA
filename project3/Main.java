...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		int[][] regions = new int[numRegionsNumLinks[0] + 1][], links = new int[numRegionsNumLinks[1] + 1][];
		
		for (int i = 1; i <= numRegionsNumLinks[0]; i++) {
			regions[i] = Arrays.stream(in.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
		}
		
		for (int i = 1; i <= numRegionsNumLinks[1]; i++) {
			links[i] = Arrays.stream(in.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
		}
		
		int safeRegion = Integer.parseInt(in.readLine());
		
		System.out.println(new RescueByRail(regions, links, safeRegion).solve());
	}
